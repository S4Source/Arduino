...
int WFSEthernetClass::begin( IPAddress local_ip)
{
...
int WFSEthernetClass::begin( IPAddress local_ip, IPAddress dns_server)
{
...
int WFSEthernetClass::begin( IPAddress local_ip, IPAddress dns_server, IPAddress gateway)
{
  IPAddress subnet(255, 255, 255, 0);
  return begin( local_ip, dns_server, gateway, subnet);
}

int WFSEthernetClass::begin( IPAddress local_ip, IPAddress dns_server, IPAddress gateway, IPAddress subnet)
{
  wifi.begin();
  ...
  if ( bJoined ) {
    ...
    if (local_ip != IPAddress(0,0,0,0) ){
      ...
      wifi.setIP(IP_ArrayToBuffer(local_ip._address, bufAddr, 16));
    }
    if (subnet != IPAddress(0,0,0,0) ){
      wifi.setNetMask(IP_ArrayToBuffer(subnet._address, bufAddr, 16));
    }
    if (gateway != IPAddress(0,0,0,1) ){
      wifi.setGateway(IP_ArrayToBuffer(gateway._address, bufAddr, 16));
    }
    if (dns_server != IPAddress(0,0,0,1) ){
      wifi.setDNS(IP_ArrayToBuffer(dns_server._address, bufAddr, 16));
    }
...
boolean WFSEthernetClass::setNTPServer( char* pNTPServer, float fTimeZoneOffsetHrs ) {
   ...
   // Set timezone adjustment: PST is -8h.  Adjust to your local timezone.
    ...
    adjustTime( (long) (fTimeZoneOffsetHrs * 60 * 60) );
    return true;
...
IPAddress WFSEthernetClass::localIP()
{
  IPAddress ret;
...
IPAddress WFSEthernetClass::subnetMask()
{
  IPAddress ret;
...
IPAddress WFSEthernetClass::gatewayIP()
{
  IPAddress ret;
  char bufAddr[SMALL_COMMAND_BUFFER_SIZE];
...
IPAddress WFSEthernetClass::dnsServerIP()
{
  IPAddress ret;
  char bufAddr[SMALL_COMMAND_BUFFER_SIZE];
